...
class PayuMoneyController extends Controller
{
    ...
    {
        if ($request->direct) {
            $payment_detail = DirectPayment::find(decrypt($request->wrap));
            $productinfo = Program::find($payment_detail->course)->name;
        } else {
            $payment_detail = PaymentProcess::find(decrypt($request->wrap));
            $productinfo = $payment_detail->program_name;
        }
        $data = $request->all();
        ...
        $posted = [
            "key" => $MERCHANT_KEY,
            "txnid" => $payment_detail->txnid,
            "amount" => $payment_detail->paid,
            "firstname" => $payment_detail->name,
            "email" => $payment_detail->email,
            "productinfo" => $productinfo,
            "phone" => $payment_detail->contact,
            "surl" => '',
            "furl" => '',
            "service_provider" => '',
        ];
        ...
        if (empty($posted['txnid'])) {
            ...
            // $new_txnid = substr(hash('sha256', mt_rand() . microtime()), 0, 20);
            $txnids = json_decode(Payment::pluck('txnid'));
            while (in_array($txnid, $txnids)) {
                $txnid = substr(hash('sha256', mt_rand() . microtime()), 0, 20);
            }
        } else {
            $txnid = $posted['txnid'];
        ...
        return view(
            'payumoney.pay',
            compact('hash', 'action', 'MERCHANT_KEY', 'formError', 'txnid', 'posted', 'SALT', 'payment_detail', 'productinfo', 'state')
        );
    }
}
