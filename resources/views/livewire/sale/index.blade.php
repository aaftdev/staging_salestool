<div>
    ...
    @endif
    <!-- Delete Sale Exit -->
    <form id="sale-filter" action="#" class="p-1 mb-4 bg-white border rounded shadow" wire:submit="getOffer()">
        <div class="p-1 form-group">
            <label for="">From</label>
            <input type="date" wire:model="from" class="form-control" wire:change="getOffer()">
        </div>
        <div class="p-1 form-group">
            <label for="">To</label>
            <input type="date" wire:model="to" class="form-control" wire:change="getOffer()">
        </div>
        <div class="p-1 form-group">
            <label for="">Search Name</label>
            <input type="name" wire:model.lazy="name" class="form-control" wire:change="getOffer()">
        </div>
        <div class="p-1 form-group">
            <label for="">Search Email</label>
            <input type="email" wire:model.lazy="email" class="form-control" wire:change="getOffer()">
        </div>
        <div class="p-1 form-group">
            <label for="">Search Contact</label>
            <input type="number" wire:model.lazy="contact" class="form-control" wire:change="getOffer()">
        </div>
        <div class="p-1 form-group">
            ...
            <div class="dropdown w-100">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                    ...
                    <li class="d-flex align-items-center justify-content-start"><input type="checkbox"
                            wire:click="getOffer" wire:model.defer="program.{{ $prog->id }}"
        ...
        <div class="p-1 form-group">
            ...
            <select wire:model="location" class="form-control" wire:change="getOffer">
        ...
        <div class="p-1 form-group">
            ...
            <select wire:model="status" class="form-control" wire:change="getOffer">
        ...
        {{-- <div class="p-1 form-group">
            ...
            <button class="btn btn-danger" type="button" wire:click="getOffer"><i class="bx bx-search"></i></button>
        </div> --}}
        {{-- <div class="p-1 form-group">
            <label for="">Counsellor</label>
            <select class="form-control" wire:model="counsellor" multiple>
                <option value="" selected>Please Select Counsellor</option>
                @foreach($counsellors as $counsel)
                <option value="{{ $counsel->counsellor }}">{{ $counsel->counsellor }}</option>
        @endforeach
        </select>
</div>
<div class="p-1 form-group">
...
@endif
<div class="text-center alert alert-info w-100" wire:loading wire:target="getOffer" role="alert">
    Searching...
</div>
    ...
    <table class="table bg-white border rounded shadow table-hover table-show">
        ...
        <tbody>
            ...
            <tr scope="row">
                ...
                <td scope="col" data-toggle="tooltip" title="{{ round($sale->final_amount) - round($paid_amount)- round($sale->otd) }}">
                    {{ round($sale->final_amount) - round($paid_amount)- round($sale->otd) }}
                </td>
                <td scope="col" data-toggle="tooltip">
                    @switch($sale->status)
                    @case(0)
