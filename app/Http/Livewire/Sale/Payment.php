...
use App\Models\Payment as NewPayment;
...
{
    public $sale, $name, $email, $contact, $program_id, $fees, $program_name, $sale_id, $payment, $batch, $otd, $program_term;
    public $amounts = [];
    public $total = 0;
    public $gst = 0;
    public $other = 0;
    ...
    public function mount($sale)
    {
        ...
        $this->remaining_amount = array_sum($this->fees) - $this->paid_total;
        if (!empty($this->otd)) {
            $this->remaining_amount -= $this->otd;
        }
    }
    ...
    public function getAmount()
    {
        if (is_numeric($this->other)) {
            ...
            if ((array_sum($this->fees) - $this->paid_total) < ($this->total + $this->otd)) {
    ...
    public function save()
    {
        ...
        $payment_process = PaymentProcess::create([
            "name" => $this->name,
            "email" => $this->email,
        ...
        return redirect()->to('/payu-money-payment?wrap=' . encrypt($payment_process->id));
    }
}
